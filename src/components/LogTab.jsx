...
function emptyProject(editorName, editorEmail, lastComposer, lastPublisher) {
  return {
    id: crypto.randomUUID(),
...
export default function LogTab({ session, editorName, onSaved, editingEntry, onCancelEdit, showToast }) {
  const [lastComposer, setLastComposer] = useState('');
  const [lastPublisher, setLastPublisher] = useState('');
  const [projects, setProjects] = useState(() =>
    editingEntry ? [projectFromEntry(editingEntry)] : [emptyProject(editorName, session.user.email, '', '')]
  );
  const [saving, setSaving] = useState(false);
  const fileInputRefs = useRef({});

  function projectFromEntry(entry) {
    // Editing someone's entry (e.g. an admin fixing a typo) never changes who
    // gets credited for logging it — that stays exactly as originally saved.
    return {
      id: crypto.randomUUID(),
      date: entry.date || new Date().toISOString().slice(0, 10),
      production: entry.production,
      editorName: entry.editor_name,
      editorEmail: entry.editor_email,
  ...
  function addProject() {
    setProjects((prev) => [...prev, emptyProject(editorName, session.user.email, lastComposer, lastPublisher)]);
  }
    ...
    try {
      ...
      for (const p of projects) {
        for (const t of p.tracks) {
          ...
          // created_by is set once, on the original insert, and never touched again —
          // editing someone's entry (e.g. an admin fixing a typo) must not silently
          // reassign ownership of that row to whoever's doing the fixing.
          if (!isEditing) row.created_by = session.user.id;
          rows.push(row);
        }
      }
      ...
      if (isEditing) {
        ...
        setProjects([emptyProject(editorName, session.user.email, last?.composers || '', last?.publishers || '')]);
      }
    } catch (err) {
      ...
      {projects.map((p, pIdx) => (
        <div className="project-block" key={p.id}>
          ...
          <div className="grid">
            ...
            <div className="field">
              <label>Editor</label>
              <input type="text" value={p.editorName} disabled title="Tied to your signed-in account" />
            </div>
            <div className="field">
