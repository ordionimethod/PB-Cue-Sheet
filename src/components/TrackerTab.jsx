...
export default function TrackerTab({ entries, onEdit, onDelete, onAddLink }) {
  ...
  const editors = useMemo(
    () => [...new Set(entries.map((e) => e.editor_email).filter(Boolean))].sort(),
    [entries]
  );
  ...
  const rows = useMemo(() => {
    ...
    if (editorFilter) r = r.filter((e) => e.editor_email === editorFilter);
    ...
    if (search.trim()) {
      ...
      r = r.filter(
        (e) =>
          ...
          esc(e.editor_email).toLowerCase().includes(q) ||
          ...
          {rows.length === 0 ? (
            ...
            rows.map((e) => (
              <tr key={e.id} className={e.pro === 'SESAC' ? 'sesac' : ''}>
                ...
                <td data-label="Editor">{esc(e.editor_email)}</td>
