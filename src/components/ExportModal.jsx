...
function buildCsv(rows) {
  ...
  rows
    ...
    .forEach((e) => {
      const row = [e.date, e.production, e.editor_email, e.exhibition, e.link, e.track, e.duration, e.usage, e.composers, e.publishers, e.pro, e.shares];
...
export default function ExportModal({ entries, editors, projects, onClose }) {
  ...
  function handleExport() {
    ...
    if (checkedEditors.length) rows = rows.filter((e) => checkedEditors.includes(e.editor_email));
