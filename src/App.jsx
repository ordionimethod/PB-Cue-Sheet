...
export default function App() {
  ...
  const [profile, setProfile] = useState(undefined); // undefined = loading, object once fetched
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!data) {
        // The auto-create trigger should have made this row already — but if it
        // hasn't landed yet (or the trigger failed for some reason), fall back
        // to creating it ourselves rather than leaving the user stuck.
  ...
  if (profile === undefined) return null; // profile still loading
  if (!profile.full_name) return <NamePrompt onSave={saveName} />;
